...
export function useCrmReports(filters: ReportsFilters) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: dealRows, error: dErr } = await dq;
      if (dErr) throw dErr;
      ...
      // Activities: range-bounded by completed_at OR created_at to keep payload small
      let aq = supabase
        .from('crm_activities')
        .select(
          `*,
      ...
      const { data: actRows, error: aErr } = await aq;
      if (aErr) throw aErr;
